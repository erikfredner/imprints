#!/usr/bin/env python3
"""
Plot stacked bar chart of proportion of PS imprints by "University Press" publishers
...
def main():
    parser = argparse.ArgumentParser(
        description="Stacked bar of PS imprints proportion by University Press publishers (10-year bins)"
    )
    parser.add_argument(
        "--input-csv",
    ...
    parser.add_argument(
        "--output",
        default="./viz/ps_university_press_binned_fraction.png",
        help="Output path for the stacked fraction chart (default: ./viz/ps_university_press_binned_fraction.png)",
    )
    ...
    # Identify top-N publishers overall
    ...
    df_up['bin'] = pd.cut(
        df_up['year_min'], bins=bins, labels=labels, right=False
    )

    # Pivot counts
    grouped = df_up.groupby(['bin', 'press_group']).size().unstack(fill_value=0)
    ...
    frac = data.div(data.sum(axis=1), axis=0) * 100

    # Plot stacked fraction bar chart
    ...
    for idx, press in enumerate(cols):
        ...
        bottom += frac[press].values
    ax.set_xlabel('Year')
    ax.set_ylabel('Percentage of PS imprints')
    ax.set_title('Proportion of PS imprints by University Presses (10-year bins)')
    ...
    print(f"Saved stacked fraction chart to: {args.output}")
