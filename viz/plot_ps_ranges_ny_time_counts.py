#!/usr/bin/env python3
"""
Plot annual counts of PS-class imprints published in New York
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot time series of NY counts for PS subranges"
    )
    parser.add_argument(
        "--input-csv",
    ...
    parser.add_argument(
        "--output",
        default="./viz/ps_ranges_time_counts.png",
    ...
    # Prepare dataframe for counts
    years = list(range(args.start_year, args.end_year + 1))
    ts = pd.DataFrame(index=years)
    for label, (low, high) in ranges.items():
        df_range = df[(df['class_digits'] >= low) & (df['class_digits'] <= high)]
        counts = df_range[df_range['in_city']].groupby('year_min').size()
        ts[label] = counts.reindex(years, fill_value=0)
    ...
    plt.xlabel('Year')
    plt.ylabel(f'Number of PS imprints in {args.city}')
    plt.title('Annual PS imprints in New York by classification range')
    plt.legend(title='Range')
    plt.tight_layout()
    os.makedirs(os.path.dirname(args.output), exist_ok=True)
    plt.savefig(args.output, dpi=600)
    print(f"Saved counts time series plot to: {args.output}")
